...
@Component({
  ...
  styleUrls: ['./questionnaire-statistics.component.css']
})
export class QuestionnaireStatisticsComponent implements OnInit {
  ...
  updateTable(start, limit, searchText) {
    ...
    this.questionaireService.getAllQuestionaireStatistics(start, limit, searchText).subscribe(
      res => {
        //console.log(res)
        this.dataLength = res['totalCount'];

        this.dataSource = res['numberOfParticipantsPerQuestionnaire'];
        //this.dataSource = res;
  ...
  viewAllQuestions(data) {
    ...
    if (this.authSerivce.getUserRoles().filter(item => config.authRoles.Questionaire.includes(item.moduleName))[0].moduleRoles.filter(i => i.name == config.rolesAction.viewStatistics).length == 0) {
      ...
      this.router.navigateByUrl("/questions-statistics"
        , { state: { questionnaireId: data.questionnaireid, name: data.name } });
    }
  }

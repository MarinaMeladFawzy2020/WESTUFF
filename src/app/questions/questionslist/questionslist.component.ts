...
export class QuestionslistComponent implements OnInit {
  ...
  constructor(public dialog: MatDialog,private service:QuestionnaireService, private snack: MatSnackBar,private activatedRoute: ActivatedRoute,private route: Router) {
...
this.service.getQuestionnaireInfoById(this.questionnaireId).subscribe(
  ...
  Response=>{
    ...
    this.myGroup.get('publishDate').setValue(this.questionnaire.publishDate);
    this.myGroup.get('expiryDate').setValue(this.questionnaire.expiryDate);
...
//this.updateTable(this.start, this.pageLength, '', this.branchManagerId);
  ...
  openDialog(isNew?,id?): void {
    ...
    const dialogRef = this.dialog.open(QuestionModalComponent, {
      width: '700px',
      data: { title: title, payload: data ,new:isNew,questionId:id,questionnaireId:this.questionnaireId}
    });
  ...
  EditQuestionaire()
  {
    ...
    this.editedQuestionnaire = {
      ...
      editable:true,
      published: false,
      createdBy: 'Nabih.Fouda.Nabeh'
    }
    this.service.editQuestionnaireInfo(this.editedQuestionnaire).subscribe(
      res => {
        //console.log(res)
        this.dataLoaded = true;
        this.actionDone = true;
        this.returnedMsg = "Questionnaire Edited Succssfully"
        this.goToDivMsg();
      }, e => {
        this.dataLoaded = true;
        this.serviceError = true;

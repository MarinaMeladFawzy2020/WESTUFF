...
export class SurveyStatisticsComponent implements OnInit {
  ...
  updateTable(start, limit, searchText) {
    ...
    this.surveyService.getAllSurveyStatistics(start, limit, searchText).subscribe(
      res => {
        //console.log(res)
        this.dataLength = res['totalCount'];

        this.dataSource = res['numberOfParticipantsPerSurvey'];
        //this.dataSource = res;
  ...
  viewAllSurvey(data) {
    ...
    if (this.authSerivce.getUserRoles().filter(item => config.authRoles.Survey.includes(item.moduleName))[0].moduleRoles.filter(i => i.name == config.rolesAction.viewStatistics).length == 0) {
      ...
      this.router.navigateByUrl("/viewsurvey"
        , { state: { surveyId: data.surveyid, name: data.name } });
    }
  }

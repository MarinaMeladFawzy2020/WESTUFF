...
@Component({
  ...
  styleUrls: ['./voting-statistics.component.css']
})
export class VotingStatisticsComponent implements OnInit {
  ...
  updateTable(start, limit, searchText) {
    ...
    this.Service.getAllVotingParticipation(start, limit, searchText).subscribe(
      res => {
        //console.log(res)
        this.dataLength = res['totalCount'];

        this.dataSource = res['numberOfParticipantsPerVoting'];
        //this.dataSource = res;
  ...
  viewAllQuestions(data) {
    ...
    if (this.authSerivce.getUserRoles().filter(item => config.authRoles.Voting.includes(item.moduleName))[0].moduleRoles.filter(i => i.name == config.rolesAction.viewStatistics).length == 0) {
      ...
      //console.log('edit' + data.questionnaireid)
      this.router.navigateByUrl("/votingStatisticsById"
        , { state: { votingId: data.votingid, name: data.name } });
    }
  }
